#!/usr/bin/env python3
"""
Variance-filter RNA + clinical + pathology Cox model with low-rank bilinear fusion.
...
Fusion:
    low-rank pairwise bilinear interactions:
    pathology x RNA, pathology x clinical, RNA x clinical -> Cox head
"""
...
DEFAULT_OUT = DEFAULT_INPUT_DIR / "outputs" / "lowrank_bilinear_uni_cv"
...
class LowRankBilinearFusion(nn.Module):
    def __init__(self, dim1: int, dim2: int, rank: int, out_dim: int):
        super().__init__()
        # Low-rank projection: map each modality into the same compact rank-dimensional space.
        # The rank controls interaction capacity; smaller rank reduces parameters.
        self.proj1 = nn.Linear(dim1, rank)
        self.proj2 = nn.Linear(dim2, rank)
        # Output projection: map the rank-dimensional interaction to the requested output size.
        self.out = nn.Linear(rank, out_dim)

    def forward(self, x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
        # Bilinear interaction: element-wise product after low-rank projection.
        # For unbatched inputs this is (rank,); for batched inputs this is (batch, rank).
...
class PairwiseLowRankBilinearFusion(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Output dimensions are chosen to sum to 640, matching the baseline concat size:
        # RNA 256 + clinical 128 + pathology 256 = 640.
        self.pathology_rna = LowRankBilinearFusion(pathology_dim, rna_dim, rank, out_dim=256)
        self.pathology_clinical = LowRankBilinearFusion(pathology_dim, clinical_dim, rank, out_dim=192)
        self.rna_clinical = LowRankBilinearFusion(rna_dim, clinical_dim, rank, out_dim=192)
        self.output_dim = 256 + 192 + 192
    ...
    def forward(
        ...
        # Pairwise outputs:
        # pathology x RNA -> 256 dims
        # pathology x clinical -> 192 dims
        # RNA x clinical -> 192 dims
...
class RNACLinicalPathologyCox(nn.Module):
    def __init__(
        ...
        self.fusion = PairwiseLowRankBilinearFusion(
            pathology_dim=he_emb_dim,
            rna_dim=rna_extractor.output_dim,
            clinical_dim=clinical_emb_dim,
            rank=64,
        )
        fusion_in = self.fusion.output_dim
    ...
    def forward_sample(self, rna: torch.Tensor, clinical: torch.Tensor, he_bag: torch.Tensor):
        ...
        rna_emb = self.rna_extractor(rna.unsqueeze(0)).squeeze(0)
        clin_emb = self.clinical_encoder(clinical)
        z = self.fusion(he_emb, rna_emb, clin_emb)
    ...
    def forward_all(self, rna: torch.Tensor, clinical: torch.Tensor, he_bags: list[torch.Tensor], device: torch.device):
        ...
        for i, bag in enumerate(he_bags):
            ...
            fused = self.fusion(he_emb, rna_emb[i], clin_emb[i])
...
def main() -> None:
    ...
    summary = {
        ...
        "fusion": "low_rank_bilinear_pairwise",
        "clinical_source": args.clinical_source,
