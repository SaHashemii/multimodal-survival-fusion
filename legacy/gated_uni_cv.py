#!/usr/bin/env python3
"""
Variance-filter RNA + clinical + pathology Cox model with gated concat fusion.
...
Fusion:
    scalar-gated concat(RNA embedding, clinical embedding, pathology embedding) -> Cox head
"""
...
from fusion_modules import ModalityGatedConcatFusion


PROJECT_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_INPUT_DIR = PROJECT_ROOT / "5_CV"
DEFAULT_OUT = DEFAULT_INPUT_DIR / "outputs" / "gated_uni_cv"
...
class RNACLinicalPathologyCox(nn.Module):
    def __init__(
        ...
        self.fusion = ModalityGatedConcatFusion(
    ...
    def forward_sample(self, rna: torch.Tensor, clinical: torch.Tensor, he_bag: torch.Tensor):
        ...
        z, _ = self.fusion(rna_emb, clin_emb, he_emb)
    ...
    def forward_all(self, rna: torch.Tensor, clinical: torch.Tensor, he_bags: list[torch.Tensor], device: torch.device):
        ...
        for i, bag in enumerate(he_bags):
            ...
            fused, _ = self.fusion(rna_emb[i], clin_emb[i], he_emb)
...
def main() -> None:
    ...
    summary = {
        ...
        "fusion": "scalar_modality_gated_concat",
        "clinical_source": args.clinical_source,
