...
from mm_survival.models.multimodal_cox import MultimodalCoxModel
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train concat/gated/low-rank multimodal Cox models.")
    parser.add_argument("--experiment", type=Path, required=True, help="Experiment config YAML.")
...
def _fusion_config(model_cfg: dict) -> tuple[str, dict]:
    fusion_cfg = dict(model_cfg.get("fusion", {}))
    fusion_name = fusion_cfg.pop("name", "concat")
    return fusion_name, fusion_cfg
...
def main() -> None:
    ...
    fold_assignments = _load_or_make_folds(dataset, cv_cfg, output_dir, args.fold_assignments)
    n_splits = int(cv_cfg.get("n_splits", 5))
    ...
    for fold in folds_to_run:
        ...
        selected_genes = []
        if hasattr(model.rna_extractor, "get_selected_genes"):
